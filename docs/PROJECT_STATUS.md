...
| ASSET-04 | 完成正式方块规则编辑、轻量草稿、严格差异、候选校验与原子晋升链 | 待人工验证 | Codex + 用户 | 55/55 Mod、53/53 Workbench 与真实浏览器通过；待空候选 validate/promote/reload 实机确认，活动规则仍为空 |
...
1. 先执行 `docs/ASSET_MANAGER_TEST_0.1.5_TEST5.md` 的空候选校验/错误哈希/原子晋升/应急重载测试；它不引入任何正式替换规则。
