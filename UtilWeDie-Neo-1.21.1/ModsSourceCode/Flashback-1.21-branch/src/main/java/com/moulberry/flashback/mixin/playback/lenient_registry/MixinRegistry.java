    ...
    default DataResult<?> referenceHolderWithLifecycle_get(DataResult<?> original) {
        ...
        if (Flashback.isInReplay()) {
            if (this == BuiltInRegistries.PARTICLE_TYPE) {
                return DataResult.success(((Registry) this).wrapAsHolder(ParticleTypes.ASH));
            } else if (this instanceof DefaultedRegistry<?> defaultedRegistry) {
                return DataResult.success(((Registry) defaultedRegistry).wrapAsHolder(defaultedRegistry.get(defaultedRegistry.getDefaultKey())));
            }
        }
        return original;
    }
