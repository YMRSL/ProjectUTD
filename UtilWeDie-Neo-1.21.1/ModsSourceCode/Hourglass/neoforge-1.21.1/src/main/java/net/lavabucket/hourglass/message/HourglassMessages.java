...
public class HourglassMessages {
     ...
     */
    @SubscribeEvent
    public static void onSleepingCheckEvent(CanContinueSleepingEvent event) {
        TimeService service = TimeServiceManager.service;
        LivingEntity entity = event.getEntity();

        if (SERVER_CONFIG.enableSleepFeature.get() == true
                && entity.getClass() == ServerPlayerWrapper.playerClass
                && ((net.minecraft.world.entity.player.Player) entity).getSleepTimer() == 2
                && service != null
                && service.level.get().equals(entity.level())
                && service.level.get().players().size() > 1
                && service.level.daylightRuleEnabled()) {

            sendEnterBedMessage(new ServerPlayerWrapper((net.minecraft.world.entity.player.Player) entity));
        }
    }
