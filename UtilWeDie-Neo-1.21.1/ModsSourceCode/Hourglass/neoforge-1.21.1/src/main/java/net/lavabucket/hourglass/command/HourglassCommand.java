...
public class HourglassCommand {
    ...
    public static <T> void onModifySuccess(CommandContext<CommandSourceStack> context, ConfigCommandEntry<T> entry) {
        // NeoForge automatically synchronizes SERVER config changes to connected clients, so the
        // explicit Forge-era config push (ConfigSynchronizer) is no longer required.
