...
public class SoulFireProjectileRenderer extends GeoEntityRenderer<SoulFireProjectileAttackEntity> {
    ...
    @Override
    public void actuallyRender(PoseStack poseStack, SoulFireProjectileAttackEntity animatable, BakedGeoModel model, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, int colour) {
        poseStack.pushPose();
        ...
        if (!isReRender) {
            ...
            AnimationState<SoulFireProjectileAttackEntity> animationState = new AnimationState(animatable, 0, 0, partialTick, avgVelocity >= motionThreshold);
            long instanceId = this.getInstanceId(animatable);
            GeoModel<SoulFireProjectileAttackEntity> currentModel = this.getGeoModel();
