...
import com.github.sculkhorde.client.model.enitity.SmallBroodAcidProjectileModel;
...
public class SmallBroodAcidProjectileRenderer extends GeoEntityRenderer<SmallBroodAcidProjectileEntity> {
    public SmallBroodAcidProjectileRenderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new SmallBroodAcidProjectileModel());
    }

    @Override
    public void actuallyRender(PoseStack poseStack, SmallBroodAcidProjectileEntity animatable, BakedGeoModel model, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, int colour) {
        poseStack.pushPose();
        ...
        if (!isReRender) {
            ...
            AnimationState<SmallBroodAcidProjectileEntity> animationState = new AnimationState(animatable, 0, 0, partialTick, avgVelocity >= motionThreshold);
            long instanceId = this.getInstanceId(animatable);
            GeoModel<SmallBroodAcidProjectileEntity> currentModel = this.getGeoModel();
