    ...
    protected void generate() {
        for (Pair<DeferredHolder<Block, ? extends Block>, ResourceLocation> pair : ModBlocks.BLOCKS_TO_DATAGEN) {
            if (pair.getA().get() instanceof SlabBlock) {
                this.add(pair.getA().get(), createSlabItemTableWithNoExplosionDecay(pair.getA().get()));
            } else {
                this.add(pair.getA().get(), createSingleItemTableWithNoExplosionDecay(pair.getA().get()));
            }
        }
    }
