...
public class DiseasedCystsEffect extends MobEffect {
    ...
    public boolean applyEffectTick(LivingEntity sourceEntity, int amp) {
        ...
        if(!entities.isEmpty())
        {
            ...
            {
                if(EntityAlgorithms.isInvalidTargetForSculkHorde(victim))
                {
                    continue;
                }
                ...
                EntityAlgorithms.applyEffectToTarget(victim, ModMobEffects.DISEASED_CYSTS, TickUnits.convertSecondsToTicks(60), SculkHorde.gravemind.getPotionAmplificationBasedOnGravemindState());


                if(victim.getHealth() <= victim.getMaxHealth() * 0.5)
                {
                    EntityAlgorithms.applyEffectToTarget(victim, ModMobEffects.SCULK_INFECTION, TickUnits.convertSecondsToTicks(10), SculkHorde.gravemind.getPotionAmplificationBasedOnGravemindState());
                    continue;
                }
