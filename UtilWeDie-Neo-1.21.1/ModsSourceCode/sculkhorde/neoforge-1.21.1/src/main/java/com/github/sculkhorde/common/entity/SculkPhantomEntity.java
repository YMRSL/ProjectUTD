...
public class SculkPhantomEntity extends FlyingMob implements GeoEntity, ISculkSmartEntity {
    ...
    protected class FallToTheGroundIfMobsUnder extends Goal
    {
        ...
        public boolean canUse()
        {
            ...
            {
                boolean isValidTarget = ((ISculkSmartEntity) SculkPhantomEntity.this).getTargetParameters().isEntityValidSculkHordeTarget(mob);
                if(isValidTarget)
                {
                    return true;
                }
    ...
    class SweepAttackGoal extends MoveTargetGoal {
        ...
        public void tick() {
            ...
            if(doesPhantomIntersectTarget)
            {
                ...
                {
                    EntityAlgorithms.applyEffectToTarget(target, ModMobEffects.SCULK_INFECTION, TickUnits.convertSecondsToTicks(30), SculkHorde.gravemind.getPotionAmplificationBasedOnGravemindState());
                }
                else if(DifficultyUtil.isCurrentDifficultyNormal())
                {
                    EntityAlgorithms.applyEffectToTarget(target, ModMobEffects.DISEASED_CYSTS, TickUnits.convertSecondsToTicks(10), SculkHorde.gravemind.getPotionAmplificationBasedOnGravemindState());
                }
                else if(DifficultyUtil.isCurrentDifficultyHard())
                {
                    EntityAlgorithms.applyEffectToTarget(target, ModMobEffects.DISEASED_CYSTS, TickUnits.convertSecondsToTicks(15), SculkHorde.gravemind.getPotionAmplificationBasedOnGravemindState());
                }
