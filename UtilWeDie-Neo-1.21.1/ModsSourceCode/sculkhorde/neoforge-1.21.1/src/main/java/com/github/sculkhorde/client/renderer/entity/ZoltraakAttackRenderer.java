...
public class ZoltraakAttackRenderer extends GeoEntityRenderer<ZoltraakAttackEntity> {
    ...
    @Override
    public void actuallyRender(PoseStack poseStack, ZoltraakAttackEntity animatable, BakedGeoModel model, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, int colour) {
        poseStack.pushPose();
        ...
        if (!isReRender) {
            ...
            AnimationState<ZoltraakAttackEntity> animationState = new AnimationState(animatable, 0, 0, partialTick, avgVelocity >= motionThreshold);
            long instanceId = this.getInstanceId(animatable);
            GeoModel<ZoltraakAttackEntity> currentModel = this.getGeoModel();
