...
public class SoulFlySwatterProjectileRenderer extends GeoEntityRenderer<SoulFlySwatterProjectileAttackEntity> {
    ...
    @Override
    public void actuallyRender(PoseStack poseStack, SoulFlySwatterProjectileAttackEntity animatable, BakedGeoModel model, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, int colour) {
        poseStack.pushPose();
        ...
        if (!isReRender) {
            ...
            AnimationState<SoulFlySwatterProjectileAttackEntity> animationState = new AnimationState(animatable, 0, 0, partialTick, avgVelocity >= motionThreshold);
            long instanceId = this.getInstanceId(animatable);
            GeoModel<SoulFlySwatterProjectileAttackEntity> currentModel = this.getGeoModel();
