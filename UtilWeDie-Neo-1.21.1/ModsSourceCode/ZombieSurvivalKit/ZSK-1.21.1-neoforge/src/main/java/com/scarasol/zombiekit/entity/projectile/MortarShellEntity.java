...
public class MortarShellEntity extends ModProjectile implements NoAttenuationProjectile, GeoEntity {
    ...
    public void burn() {
        ...
        level.getEntitiesOfClass(LivingEntity.class, new AABB(getOnPos()).inflate(CommonConfig.NAPALM_RANGE.get()), (target) -> !target.fireImmune())
                .forEach((target) -> {
                    ...
                    target.addEffect(new MobEffectInstance(SonaMobEffects.IGNITION, time, 3, false, false));
                    target.addEffect(new MobEffectInstance(SonaMobEffects.SLIMINESS, time, 0, false, false));
                });
        this.discard();
    }
        ...
        if (level() instanceof ServerLevel serverLevel) {
            if (!firstHit) {
                ...
                level().getEntitiesOfClass(LivingEntity.class, new AABB(getOnPos()).inflate(10D))
                        .forEach((target) -> {
                            target.addEffect(new MobEffectInstance(SonaMobEffects.CAMOUFLAGE, 20, 4, false, false));
                            if (!(target instanceof Player))
                                target.addEffect(new MobEffectInstance(SonaMobEffects.CONFUSION, 20, 0, false, false));
                        });
                if (lifeTime++ > 1000) {
                    this.discard();
                }
    ...
    public void doEffects(Level level, double x, double y, double z) {
        if (level() instanceof ServerLevel serverLevel) {
            if (this.launchSchedule != null) {
                ...
                serverLevel.getChunkSource().removeRegionTicket(TicketType.FORCED, currentChunkPos, 3, currentChunkPos);
            effect.shellEffect(this);
        }
    }
    ...
    public void updateForceRegion() {
        ...
        if (launchSchedule == null && !pos.equals(currentChunkPos) && this.level() instanceof ServerLevel serverLevel) {
            serverLevel.getChunkSource().addRegionTicket(TicketType.FORCED, pos, 3, pos);
