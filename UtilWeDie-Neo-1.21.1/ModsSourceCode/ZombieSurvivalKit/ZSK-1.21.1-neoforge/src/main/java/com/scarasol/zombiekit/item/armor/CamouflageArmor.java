...
public abstract class CamouflageArmor extends ArmorItem {
    ...
    public CamouflageArmor(Holder<ArmorMaterial> armorMaterial, Type equipmentSlot, Properties properties, int camouflage) {
    ...
    public void inventoryTick(ItemStack itemStack, Level level, Entity entity, int slotIndex, boolean selected){
        ...
        if (getEquipmentSlot() == EquipmentSlot.CHEST && Iterables.contains(player.getArmorSlots(), itemStack)){
            if (camouflage == 0 || player.isSprinting() || player.hasEffect(SonaMobEffects.EXPOSURE))
                return;
            ...
            if (level.getMaxLocalRawBrightness(blockPos) < 7 || (level.isRaining() && level.canSeeSkyFromBelowWater(blockPos)))
                amplifier++;
            player.addEffect(new MobEffectInstance(SonaMobEffects.CAMOUFLAGE, 20, amplifier, false, false));
        }
    }
