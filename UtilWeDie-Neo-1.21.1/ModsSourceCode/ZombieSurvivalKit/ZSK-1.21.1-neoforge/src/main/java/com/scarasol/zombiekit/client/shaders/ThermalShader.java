...
public class ThermalShader implements ResourceManagerReloadListener {
    private static final ResourceLocation THERMAL_EFFECT = ResourceLocation.fromNamespaceAndPath("zombiekit", "shaders/post/thermal.json");
    ...
    // 热成像（全屏灰度夜视）仅在穿戴 ExoArmor 胸甲且雷达处于夜视档(radar==1)时启用；
    // 否则默认会一直开（上游无条件 setActive(true) 的隐患在 1.21.1 暴露为"进游戏即夜视"）。
    private static boolean shouldThermalBeActive() {
        Minecraft mc = Minecraft.getInstance();
        if (mc.player == null) {
            return false;
        }
        ItemStack chest = mc.player.getItemBySlot(EquipmentSlot.CHEST);
        return chest.getItem() instanceof ExoArmor && ExoArmor.getRadar(chest) == 1;
    }

    @SubscribeEvent
    public static void onRenderLevel(RenderLevelStageEvent event) {
        setSeeThroughWalls(false);
        setActive(shouldThermalBeActive());
        if (!isActive) {
            return;
        }
        ...
        if (event.getStage() == RenderLevelStageEvent.Stage.AFTER_PARTICLES) {
            ...
            prepareAndRenderEntities(event.getPoseStack(), event.getPartialTick().getGameTimeDeltaPartialTick(false));
        }
    }
...
//        return entity instanceof LivingEntity || entity.isOnFire();
    ...
    private static void collectVisibleLuminousBlocks() {
        ...
        ObjectArrayList<SectionRenderDispatcher.RenderSection> sections = accessor.zombiekit$getVisibleSections();
        if (sections == null) {
            return;
        }
        ...
        for (SectionRenderDispatcher.RenderSection renderSection : sections) {
