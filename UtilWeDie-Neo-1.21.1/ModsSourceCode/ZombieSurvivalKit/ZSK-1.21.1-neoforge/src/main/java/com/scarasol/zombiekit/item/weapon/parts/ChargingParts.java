...
public abstract class ChargingParts extends Item implements Parts {
    ...
    public long getCooldownTime(ItemStack itemStack) {
        if (itemStack.getItem() instanceof ModifiableWeapon && itemStack.has(ZombieKitDataComponents.RELEASE_TIME.get()))
            return itemStack.get(ZombieKitDataComponents.RELEASE_TIME.get());
        return -1L;
    }

    public void setCooldownTime(ItemStack itemStack, long releaseTime) {
        itemStack.set(ZombieKitDataComponents.RELEASE_TIME.get(), releaseTime);
    }
