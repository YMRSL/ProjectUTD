...
public class HeavyMachineGunUsingGoal<T extends Mob> extends Goal {
    ...
    private boolean isValidTarget() {
        if (this.mob.getTarget() != null && this.mob.getTarget().isAlive() && isInMachineGunSight() && !this.mob.getTarget().getType().toString().equals("entity.sona.sound_decoy")) {
            return true;
                ...
                if (this.mob.getVehicle() instanceof HeavyMachineGunEntity heavyMachineGun) {
                    if (this.attackCooldown <= 0) {
                        if (this.attackTimes-- <= 0) {
                            ...
                            if (heavyMachineGun.getOverload())
                                livingEntity.level().playSound(null, heavyMachineGun.getX(), heavyMachineGun.getY(), heavyMachineGun.getZ(), ZombieKitSounds.heavy_machine_gun_trigger.get(), SoundSource.BLOCKS, 1, 1);
                            else
    ...
    public boolean searchEntities() {
        double range = this.mob.getAttributeValue(Attributes.FOLLOW_RANGE);
        List<LivingEntity> list = this.mob.level().getNearbyEntities(LivingEntity.class, TargetingConditions.forCombat().range(range).selector(predicate), this.mob, this.mob.getBoundingBox().inflate(range))
                .stream().filter(livingEntity -> this.mob.position().distanceTo(livingEntity.position()) < range)
                .sorted((l1, l2) -> {
        ...
        if (!list.isEmpty()) {
            for (LivingEntity entity : list) {
                if (entity.getType().toString().equals("entity.sona.sound_decoy"))
                    continue;
                if (this.mob.canAttack(entity)) {
    ...
    public boolean searchPlayer() {
        double range = this.mob.getAttributeValue(Attributes.FOLLOW_RANGE);
        List<Player> list = this.mob.level().getNearbyPlayers(TargetingConditions.forCombat().range(range), this.mob, this.mob.getBoundingBox().inflate(range))
                .stream().filter(player -> this.mob.position().distanceTo(player.position()) < range)
                .sorted((l1, l2) -> {
                    double a = Math.pow(l1.getX() - HeavyMachineGunUsingGoal.this.mob.getX(), 2) + Math.pow(l1.getZ() - HeavyMachineGunUsingGoal.this.mob.getZ(), 2);
                    double b = Math.pow(l2.getX() - HeavyMachineGunUsingGoal.this.mob.getX(), 2) + Math.pow(l2.getZ() - HeavyMachineGunUsingGoal.this.mob.getZ(), 2);
                    return Double.compare(a, b);
