...
public class OverlayEvent {
        ...
        if (entity != null && ExoArmor.numberOfSuit(entity) >= 4) {
            ...
            if (!entity.isSpectator()) {
                ...
                if (ExoArmor.getRadar(itemStack) == 2){
                    ...
                    event.getGuiGraphics().blit(new ResourceLocation("zombiekit:textures/screens/exo/" + color + "/night_vision.png"), posX + 96, posY + 2, 0, 0, 8, 8, 8, 8);
                }
    ...
    public static void renderSum(GuiGraphics guiGraphics, Player player, int posX, int posY, Predicate<ItemStack> predicate) {
        ...
        for (int i = 0; i < number.length; i++) {
            ...
            if (flag) {
                guiGraphics.blit(new ResourceLocation("zombiekit:textures/screens/weapon/" + num + ".png"), posX, posY, 0, 0, 6, 10, 6, 10);
                posX += 8;
            }
        }
    }
    ...
    public static void renderMortarInfo(GuiGraphics guiGraphics, int posX, int posY, MortarEntity mortarEntity, Player player) {
        ...
        guiGraphics.blit(new ResourceLocation("zombiekit:textures/screens/weapon/mortar_bottom.png"), posX, posY, 0, 0, 28, 2, 28, 2);
        ...
        guiGraphics.blit(new ResourceLocation("zombiekit:textures/screens/weapon/mortar.png"), -20, -20, 0, 0, 30, 30, 30, 30);
        guiGraphics.pose().popPose();
        guiGraphics.pose().pushPose();
        posX = posX + 30;
        posY = posY - 28;
        guiGraphics.blit(new ResourceLocation("zombiekit:textures/screens/weapon/mortar_shell.png"), posX, posY, 0, 0, 22, 20, 22, 20);
    ...
    public static void renderHeavyMachineGunInfo(GuiGraphics guiGraphics, int posX, int posY, Player player) {
        guiGraphics.pose().pushPose();
        guiGraphics.blit(new ResourceLocation("zombiekit:textures/screens/weapon/heavy_machine_gun.png"), posX, posY, 0, 0, 56, 16, 56, 16);
    ...
    public static void renderFlamethrowerInfo(GuiGraphics guiGraphics, int posX, int posY, ItemStack itemStack) {
        ...
        guiGraphics.pose().pushPose();
        guiGraphics.blit(new ResourceLocation("zombiekit:textures/screens/weapon/flamethrower.png"), posX, posY, 0, 0, 80, 28, 80, 28);
        guiGraphics.pose().translate(posX + 61, posY + 19, 0);
        guiGraphics.pose().mulPose(Axis.ZP.rotation((float) (-Math.PI / 4 + pressure * Math.PI)));
        guiGraphics.blit(new ResourceLocation("zombiekit:textures/screens/weapon/pointer.png"), -9, -9, 0, 0, 10, 10, 10, 10);
        guiGraphics.pose().popPose();
    }
    ...
    public static void renderChargingPartsInfo(GuiGraphics guiGraphics, int posX, int posY, ItemStack itemStack) {
        if (itemStack.getItem() instanceof ModifiableWeapon modifiableWeapon) {
            ...
            if (parts instanceof ChargingParts chargingParts && chargingParts.isOnCoolDown(itemStack, level.getGameTime())) {
                guiGraphics.pose().pushPose();
                guiGraphics.blit(new ResourceLocation("zombiekit:textures/screens/weapon/charging_parts_empty.png"), posX + 10, posY - 5, 0, 0, 10, 10, 10, 10);
                double percent = (level.getGameTime() - chargingParts.getCooldownTime(itemStack)) / (double)chargingParts.getChargingTime();
                guiGraphics.blit(new ResourceLocation("zombiekit:textures/screens/weapon/charging_parts.png"), posX + 10, posY - 5, 0, 0, 10, (int) Math.round(10 * percent), 10, 10);
                guiGraphics.pose().popPose();
            }
