        ...
        public static void onKeyInput(TickEvent.ClientTickEvent event) {
            if (Minecraft.getInstance().screen == null) {
                if (MODIFICATION_GUI.consumeClick())
                    NetworkHandler.PACKET_HANDLER.sendToServer(new KeyBindPacket(6));
                else if (CHARGING_PARTS.consumeClick())
                    NetworkHandler.PACKET_HANDLER.sendToServer(new KeyBindPacket(7));
                else if (FLAMETHROWER_RELOAD.consumeClick())
                    NetworkHandler.PACKET_HANDLER.sendToServer(new KeyBindPacket(8));
                else if (ExoArmor.numberOfSuit(Minecraft.getInstance().player) == 4)
                    if (EXO_SNEAK_MODE.consumeClick()) {
                        NetworkHandler.PACKET_HANDLER.sendToServer(new KeyBindPacket(1));
                    } else if (EXO_COMBAT_MODE.consumeClick()) {
                        NetworkHandler.PACKET_HANDLER.sendToServer(new KeyBindPacket(2));
                    } else if (EXO_REACTIVE_ARMOR.consumeClick()) {
                        NetworkHandler.PACKET_HANDLER.sendToServer(new KeyBindPacket(3));
                    } else if (EXO_RADAR.consumeClick())
                        NetworkHandler.PACKET_HANDLER.sendToServer(new KeyBindPacket(4));
                    else if (EXO_FLY_MODE.consumeClick())
                        NetworkHandler.PACKET_HANDLER.sendToServer(new KeyBindPacket(5));
            }
        }
    }
