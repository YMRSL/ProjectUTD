...
public class FuelCanister extends Item implements BaseFuelCanister {
    ...
    public void canisterEffect(LivingEntity target) {
        ...
        if (fuelType == FuelType.NORMAL) {
            ...
            target.addEffect(new MobEffectInstance(SonaMobEffects.IGNITION.get(), 110, 1, false, false));
        }else if (fuelType == FuelType.HIGH_TEMPERATURE) {
            damage = CommonConfig.HIGH_TEMPERATURE_FUEL_DAMAGE.get();
            target.addEffect(new MobEffectInstance(SonaMobEffects.IGNITION.get(), 150, 3, false, false));
        }else if (fuelType == FuelType.NAPALM) {
            damage = CommonConfig.NAPALM_FUEL_DAMAGE.get();
            target.addEffect(new MobEffectInstance(SonaMobEffects.IGNITION.get(), 230, Math.max(damage - 1, 1), false, false));
            target.addEffect(new MobEffectInstance(SonaMobEffects.SLIMINESS.get(), 230, 0, false, false));
        }
