...
public final class SuperbTurretCompat {
    ...
    private static Field turretYRotOField;
    private static Field turretXRotOField;
    ...
    public static void setTurretYRotO(Entity entity, float yaw) {
        if (!isVehicle(entity) || turretYRotOField == null) {
            return;
        }
        try {
            turretYRotOField.setFloat(entity, yaw);
        } catch (IllegalAccessException e) {
            VehicleLoadMod.LOGGER.debug("SuperbTurretCompat: turretYRotO set failed", e);
        }
    }

    public static void setTurretXRotO(Entity entity, float pitch) {
        if (!isVehicle(entity) || turretXRotOField == null) {
            return;
        }
        try {
            turretXRotOField.setFloat(entity, pitch);
        } catch (IllegalAccessException e) {
            VehicleLoadMod.LOGGER.debug("SuperbTurretCompat: turretXRotO set failed", e);
        }
    }
    ...
    private static boolean init() {
        ...
        try {
            ...
            turretYRotOField = vehicleClass.getField("turretYRotO");
            turretXRotOField = vehicleClass.getField("turretXRotO");
            return true;
        } catch (ReflectiveOperationException e) {
