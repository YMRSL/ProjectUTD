...
public class ItemKangfuXinYe extends Item {
    public ItemKangfuXinYe() {
        super(new Properties().food(new FoodProperties.Builder().build()).craftRemainder(Items.GLASS_BOTTLE).stacksTo(1));
    }
    ...
    public ItemStack finishUsingItem(ItemStack pStack, Level pLevel, LivingEntity pLivingEntity) {
        ...
        List.copyOf(pLivingEntity.getActiveEffectsMap().keySet()).stream()
                .filter(it -> it.value().getCategory() == MobEffectCategory.HARMFUL)
