...
public final class TaczAmmoCompat {
    private static final ResourceLocation FALLBACK_EMPTY_AMMO_ID = ResourceLocation.fromNamespaceAndPath("tacz", "empty");
    ...
    public static int extractCompatibleAmmo(Player player, ItemStack gunStack, int neededAmount, boolean simulate) {
        ...
        extracted += extractFromStorageStack(vestStack, gunStack, neededAmount - extracted, simulate);
        if (extracted < neededAmount) {
            extracted += extractFromStorageStack(player.getItemBySlot(EquipmentSlot.CHEST), gunStack, neededAmount - extracted, simulate);
        }

        if (extracted < neededAmount) {
            extracted += extractFromStorageStack(player.getItemBySlot(EquipmentSlot.LEGS), gunStack, neededAmount - extracted, simulate);
        }
    ...
    private static int extractFromStorageStack(ItemStack storageStack, ItemStack gunStack, int neededAmount, boolean simulate) {
        ...
        int extracted = 0;
        IItemHandler handler = storageStack.getCapability(Capabilities.ItemHandler.ITEM);
        if (handler != null) {
        ...
        // 回退路径：直接读 BACKPACK_INVENTORY 组件（与 NestedStorageItemHandler 同一份内嵌库存）
        CompoundTag inventoryTag = storageStack.get(BlockZDataComponents.BACKPACK_INVENTORY.get());
        ...
        fallbackHandler.deserializeNBT(provider, inventoryTag);
        ...
        if (!simulate && extractedFromTag > 0) {
            if (hasAnyItems(fallbackHandler)) {
                storageStack.set(BlockZDataComponents.BACKPACK_INVENTORY.get(), fallbackHandler.serializeNBT(provider));
            } else {
                storageStack.remove(BlockZDataComponents.BACKPACK_INVENTORY.get());
            }
        }
     ...
     * 弹药供弹既可发生在服务端（实际消耗）也可发生在客户端（HUD 计数模拟），
     * 故服务端走运行中 server.registryAccess()，客户端回退到当前世界 registryAccess。
     */
    private static HolderLookup.Provider provider() {
        ...
        throw new IllegalStateException("BlockZ: no HolderLookup.Provider available for tacz ammo storage access");
    }

    /** 客户端 registryAccess 访问隔离到内部类，避免服务端加载 Minecraft 客户端类。 */
