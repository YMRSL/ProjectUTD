...
public interface IInfectionZoneManager {
    ...
    default boolean canChunkInfection(Level level) {
        return CommonConfig.INFECTION_OPEN.get() && CommonConfig.INFECTED_ZONE_OPEN.get() && level.dimensionTypeRegistration().is(BuiltinDimensionTypes.OVERWORLD);
    }
    ...
    default ChunkPos chunkPosOffset(WorldGenLevel level, ChunkPos chunkPos) {
        ...
        if (zeroBlockPos == null) {
            ...
            zeroBlockPos = calculateZeroZone(new BlockPos(spawnPos.getX(), spawnPos.getY(), spawnPos.getZ()));
