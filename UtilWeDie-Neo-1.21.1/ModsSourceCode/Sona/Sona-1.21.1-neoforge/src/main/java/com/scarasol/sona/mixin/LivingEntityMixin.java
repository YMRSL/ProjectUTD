...
public abstract class LivingEntityMixin extends Entity implements ILivingEntityAccessor, ISonaDataAccessor {
    ...
    @Shadow
    public abstract boolean hasEffect(net.minecraft.core.Holder<net.minecraft.world.effect.MobEffect> p_21024_);

    @Shadow
    public abstract double getAttributeValue(net.minecraft.core.Holder<net.minecraft.world.entity.ai.attributes.Attribute> p_21134_);

    @Shadow
    public abstract net.minecraft.world.entity.ai.attributes.AttributeInstance getAttribute(net.minecraft.core.Holder<net.minecraft.world.entity.ai.attributes.Attribute> p_21051_);

    @Shadow public abstract boolean hurt(DamageSource p_21016_, float p_21017_);

    @Shadow public abstract MobEffectInstance getEffect(net.minecraft.core.Holder<net.minecraft.world.effect.MobEffect> p_21125_);
    ...
    private void onDefineSynchedData(SynchedEntityData.Builder builder, CallbackInfo ci) {
        builder.define(INFECTION_LEVEL, 0F);
        builder.define(INJURY_LEVEL, 100F);
        builder.define(BANDAGE_LEVEL, 0F);
        builder.define(CAMOUFLAGE_AMPLIFIER, 0);
        builder.define(EXPOSURE_AMPLIFIER, -1);
        builder.define(INFECTION_LAYER, false);
    }
    ...
    @Inject(method = "removeAllEffects", at = @At(value = "INVOKE", target = "Lnet/minecraft/world/entity/LivingEntity;onEffectRemoved(Lnet/minecraft/world/effect/MobEffectInstance;)V"), locals = LocalCapture.CAPTURE_FAILSOFT, require = 0)
    public void onRemoveAllEffects(CallbackInfoReturnable<Boolean> cir, Iterator<MobEffectInstance> iterator, boolean flag, MobEffectInstance effect) {
        if (effect.getEffect().value() instanceof PhysicalEffect && !CommonConfig.PHYSICAL_EFFECT_REMOVE.get()) {
    ...
    private Entity warpLastHurtMob(DamageSource instance, Operation<Entity> operation) {
        if (instance.getEntity() instanceof LivingEntity livingEntity && livingEntity.getAttribute(Attributes.FOLLOW_RANGE) != null && this.getAttribute(Attributes.FOLLOW_RANGE) != null && instance.getDirectEntity() != instance.getEntity()) {
            if (livingEntity.hasEffect(SonaMobEffects.CAMOUFLAGE)) {
                double distance = this.position().distanceTo(livingEntity.position());
                if (livingEntity.hasEffect(SonaMobEffects.EXPOSURE)) {
                    double exposureRange = this.getAttributeValue(Attributes.FOLLOW_RANGE) * 0.3 * (livingEntity.getEffect(SonaMobEffects.EXPOSURE).getAmplifier() + 1);
                    if (exposureRange > distance) {
                        return operation.call(instance);
                    }
                }
                double range = this.getAttributeValue(Attributes.FOLLOW_RANGE) * (1 / Math.pow(2, livingEntity.getEffect(SonaMobEffects.CAMOUFLAGE).getAmplifier() + 1));
                if (distance > range) {
                    return null;
                }
            }
    ...
    private void sona$swing(InteractionHand interactionHand, boolean send, CallbackInfo ci) {
        if (hasEffect(SonaMobEffects.LACERATION)) {
            int proc = (Object)this instanceof Player ? 2 : 5;
            setSona$laceration(proc * (getEffect(SonaMobEffects.LACERATION).getAmplifier() + 1));
        }
    }

    @Inject(method = "travel", at = @At("HEAD"))
    private void sona$Travel(Vec3 input, CallbackInfo ci) {
        if (!level().isClientSide && !isPassenger() && hasEffect(SonaMobEffects.LACERATION)) {
            if (!((Object)this instanceof Player) && !input.equals(Vec3.ZERO)) {
                int amplifier = getEffect(SonaMobEffects.LACERATION).getAmplifier() + 1;
    ...
    private void onBaseTick(CallbackInfo ci) {
        ...
        if (level() instanceof ServerLevel) {
            if (hasEffect(SonaMobEffects.CAMOUFLAGE)) {
                int amplifier = getEffect(SonaMobEffects.CAMOUFLAGE).getAmplifier() + 1;
                setCamouflageAmplifier(amplifier);
            } else {
                setCamouflageAmplifier(0);
            }
            if (hasEffect(SonaMobEffects.EXPOSURE)) {
                setExposureAmplifier(getEffect(SonaMobEffects.EXPOSURE).getAmplifier());
            } else {
                setExposureAmplifier(-1);
            }
            if (CommonConfig.EXPOSURE_INDICATOR.get() && (Object) this instanceof LivingEntity livingEntity && hasEffect(SonaMobEffects.EXPOSURE) && gameTime % 10 == 0) {
                int amplifier = getEffect(SonaMobEffects.EXPOSURE).getAmplifier();
            ...
            if ((Object) this instanceof LivingEntity livingEntity) {
                ...
                if (!this.level().isClientSide() && flag) {
                    if (CommonConfig.INFECTION_OPEN.get()) {
                        if (gameTime % 20 == 0) {
                            if (hasEffect(SonaMobEffects.INFECTION)) {
    ...
    public void setXRot(float xRot) {
        if (!(this.hasEffect(SonaMobEffects.STUN) || (this.hasEffect(SonaMobEffects.SLIMINESS) && this.hasEffect(SonaMobEffects.FROST)))) {
            super.setXRot(xRot);
        }
    }
