...
public class SubGuiModelExtras extends GuiNPCInterface implements ITextfieldListener {
    ...
    public void init() {
        ...
        // Attack telegraph hit frame. 0 = no windup (deal damage instantly, anim is just a flourish).
        addLabel(new GuiLabel(6,"Attack Hit Tick", guiLeft - 85, y + 5,0xffffff));
        GuiTextFieldNop attackHitTick = new GuiTextFieldNop(6,this, guiLeft + 50, y,
                200, 20, ""+getModelData(npc).getAttackHitTick());
        attackHitTick.setNumbersOnly();
        attackHitTick.setMinMaxDefault(0, Integer.MAX_VALUE, getModelData(npc).getAttackHitTick());
        addTextField(attackHitTick);
        y+=23;
    ...
    public void unFocused(GuiTextFieldNop textfield) {
        ...
        if(textfield.id == 6){
            getModelData(npc).setAttackHitTick(textfield.getInteger());
        }
        if(textfield.id == 3){
